...
SELECT A.MSO,B.SCIERA_CB,A.PROVIDER_NAME,COUNT(*) AS ROW_COUNT,MIN(A.OPEN_DATE) FIBER_FIRST_FOUND FROM V4_CTELL.PROC.HKEY_CURRENT A JOIN MSO.V3_COX_RESOURCE.FP B ON A.FP_KEY=B.FP_KEY WHERE A.MSO='COX' AND A.MAX_SPEED >= 300 AND B.DELETE_DATE IS NULL GROUP BY ALL;

INSERT INTO V4_CTELL.ROLLUPS.FIBER_FOUND_CB
SELECT A.MSO,B.SCIERA_CB,A.PROVIDER_NAME,COUNT(*) AS ROW_COUNT,MIN(A.OPEN_DATE) FIBER_FIRST_FOUND FROM V4_CTELL.PROC.HKEY_CURRENT A JOIN MSO.V3_COMCAST_RESOURCE.FP B ON A.FP_KEY=B.FP_KEY WHERE A.MSO='COMCAST' AND A.MAX_SPEED >= 300 AND B.DELETE_DATE IS NULL GROUP BY ALL;

INSERT INTO V4_CTELL.ROLLUPS.FIBER_FOUND_CB
SELECT A.MSO,B.SCIERA_CB,A.PROVIDER_NAME,COUNT(*) AS ROW_COUNT,MIN(A.OPEN_DATE) FIBER_FIRST_FOUND FROM V4_CTELL.PROC.HKEY_CURRENT A JOIN MSO.V3_CHARTER_RESOURCE.FP B ON A.FP_KEY=B.FP_KEY WHERE A.MSO='CHARTER' AND A.MAX_SPEED >= 300 AND B.DELETE_DATE IS NULL GROUP BY ALL;

UPDATE V4_CTELL.ROLLUPS.CB_ROLLUP A SET A.FIBER_FIRST_FOUND=B.FIBER_FIRST_FOUND
FROM V4_CTELL.ROLLUPS.FIBER_FOUND_CB B WHERE A.MSO=B.MSO AND A.SCIERA_CB=B.SCIERA_CB AND A.PROVIDER_NAME=B.PROVIDER_NAME;  
...
select mso,zip5,zip4,provider_name,count(*) as row_count
,min(open_date) as first_check_date
from v4_ctell.proc.hkey_current
group by all;

create or replace table V4_CTELL.ROLLUPS.cb_first_check_date as 
select mso,sciera_cb,provider_name,count(*) as row_count
,min(open_date) as first_check_date
from v4_ctell.proc.hkey_current a , mso.v3_comcast_resource.fp b where a.fp_key=b.fp_key and delete_date is null
group by all;

insert into V4_CTELL.ROLLUPS.cb_first_check_date 
select mso,sciera_cb,provider_name,count(*) as row_count
,min(open_date) as first_check_date
from v4_ctell.proc.hkey_current a , mso.v3_charter_resource.fp b where a.fp_key=b.fp_key and delete_date is null
group by all;

insert into V4_CTELL.ROLLUPS.cb_first_check_date 
select mso,sciera_cb,provider_name,count(*) as row_count
,min(open_date) as first_check_date
from v4_ctell.proc.hkey_current a , mso.v3_cox_resource.fp b where a.fp_key=b.fp_key and delete_date is null
group by all;
