...
UPDATE "<<DB_NAME>>"."<<SCHEMA_NAME>>"."WIP_CURRENT"
SET  BUDGET_MONTH= CASE WHEN DAY(SCHEDULEDATE) < 25 THEN  MONTH(DATE_TRUNC('MONTH', SCHEDULEDATE )) 
                          ELSE MONTH(DATE_TRUNC('MONTH', SCHEDULEDATE + INTERVAL '1 MONTH'))  END WHERE (PROJECT_TYPE NOT IN ('NQ','WL') OR PROJECT_TYPE IS NULL) AND BUDGET_MONTH IS NULL;

UPDATE "<<DB_NAME>>"."<<SCHEMA_NAME>>"."WIP_CURRENT"
SET  BUDGET_YEAR= CASE WHEN DAY(SCHEDULEDATE) < 25 THEN  YEAR(DATE_TRUNC('MONTH', SCHEDULEDATE )) 
                          ELSE YEAR(DATE_TRUNC('MONTH', SCHEDULEDATE + INTERVAL '1 MONTH'))  END  WHERE (PROJECT_TYPE NOT IN ('NQ','WL') OR PROJECT_TYPE IS NULL)  AND BUDGET_YEAR IS NULL ;
