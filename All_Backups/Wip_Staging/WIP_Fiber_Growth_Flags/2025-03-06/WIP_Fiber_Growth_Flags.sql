...
WHERE A.MSO = B.MSO AND A.SERVICENAME = B.PROVIDER_NAME AND A.ZIP9 = B.ZIP9 AND B.FIRST_CHECK_DATE_FL < A.CHECK_FL_DATE AND A.BUDGET_MONTH = MONTH(CURRENT_DATE) AND A.BUDGET_YEAR = YEAR(CURRENT_DATE);

UPDATE V4_CTELL.PROC.WIP_CURRENT A
SET A.FIRST_CHECK_DATE_CB = B.FIRST_CHECK_DATE
FROM V4_CTELL.ROLLUPS.CB_CHECKED_ROLLUP B
WHERE A.MSO = B.MSO AND A.SERVICENAME = B.PROVIDER_NAME AND A.CB = B.SCIERA_CB AND B.FIRST_CHECK_DATE_FL < A.CHECK_FL_DATE AND A.BUDGET_MONTH = MONTH(CURRENT_DATE) AND A.BUDGET_YEAR = YEAR(CURRENT_DATE);
