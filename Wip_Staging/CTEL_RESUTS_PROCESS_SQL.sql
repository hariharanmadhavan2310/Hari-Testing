...
/*MAX_SPEED_CHANGED COLUMN HAS BEEN UPDATED FROM TABLE INSTAED OF MANUAL UPDATION*/
/*ADD AGGREGATED_MAX_SPEED COLUMN*/

UPDATE "<<DB_NAME>>"."<<SCHEMA_NAME>>"."RAW_RESULTS_STAGING" A 
SET A.MAX_SPEED_CHANGED = B.MAX_SPEED_CHANGED
FROM "V4_CTELL"."TEST"."SPEEDS_NEED_TO_CHANGE" B
WHERE A.SERVICE_PROVIDER = B.SERVICE_PROVIDER AND A.MAX_SPEED = B.MAX_SPEED;

UPDATE "<<DB_NAME>>"."<<SCHEMA_NAME>>"."RAW_RESULTS_STAGING"
SET MAX_SPEED_CHANGED = MAX_SPEED
WHERE MAX_SPEED_CHANGED IS NULL;
...
UPDATE "<<DB_NAME>>"."<<SCHEMA_NAME>>"."WIP_CURRENT" A 
    SET A.STATUS = B.STATUS,
		...
		A.AGGREGATED_MAX_SPEED = B.AGGREGATED_MAX_SPEED,		
...
INSERT INTO "<<DB_NAME>>"."<<SCHEMA_NAME>>"."WIP_CURRENT" (MSO,SERVICENAME,FP_KEY,PROJECTNAME,SCHEDULEDATE,STATUS,CHECK_DATE,AGGREGATED_MAX_SPEED,MAX_SPEED,UPLOAD_MAX_SPEED,DELIVERED_DATE,INPUT_FILE_NAME,INPUT_FILE_DATE,RESULT_FILE_NAME,RESULT_FILE_DATE,RESULT_SOURCE,PROCESSED_DATE) SELECT A.MSO,A.PROVIDER_NAME,A.FP_KEY,A.PROJECT_NAME,CASE WHEN A.SCHEDULE_DATE IS NULL THEN A.CHECK_DATE ELSE A.SCHEDULE_DATE END AS SCHEDULE_DATE,A.STATUS,A.CHECK_DATE,A.AGGREGATED_MAX_SPEED,A.MAX_SPEED,A.UPLOAD_MAX_SPEED,NULL,A.RESULT_FILENAME,A.RESULT_FILEDATE,A.RESULT_FILENAME,A.RESULT_FILEDATE,A.RESULT_SOURCE,CURRENT_DATE()
...
/*T MOBILE FIBER REMOVED FROM FWA*/
...
SET PROJECT_TYPE = CASE WHEN UPPER(SUBSTR(PROJECTNAME,1,1))='B'  THEN 'BL'
						...
						WHEN UPPER(SUBSTR(PROJECTNAME,1,2))='LL' THEN 'LL'
                        ELSE NULL END
...
UPDATE V4_CTELL.PROC.WIP_CURRENT
SET BUDGET_YEAR = QUEUE_YEAR
WHERE BUDGET_YEAR IS NULL;

UPDATE V4_CTELL.PROC.WIP_CURRENT
SET BUDGET_MONTH = QUEUE_MONTH
WHERE BUDGET_MONTH IS NULL;

/*DATASET FOR DASHBOARDS HAVE BEEN ADDED*/

CREATE OR REPLACE TABLE "SCIERA_SUBSYSTEM"."QUICKSIGHT"."WIP_SPEEDS_PROVIDER_WISE" AS
SELECT MSO,SERVICENAME,MAX_SPEED,CHECK_DATE,RESULT_FILE_DATE,RESULT_SOURCE,COUNT(*) AS COUNT FROM V4_CTELL.PROC.WIP_CURRENT GROUP BY ALL;

CREATE OR REPLACE TABLE "SCIERA_SUBSYSTEM"."QUICKSIGHT"."CTEL_AGGREGATED_RESULTS" AS
SELECT MSO,SERVICE_PROVIDER,RESULT_FILEDATE,RESULT_SOURCE,PROVIDERTYPE,COUNT(*) TOTAL_COUNTS FROM V4_CTELL.PROC.RAW_RESULTS_STAGING GROUP BY ALL;

TRUNCATE TABLE SCIERA_SUBSYSTEM.QUICKSIGHT.WIP_RESULTS_DASHBOARD;

INSERT INTO SCIERA_SUBSYSTEM.QUICKSIGHT.WIP_RESULTS_DASHBOARD
SELECT MSO, SERVICENAME, FP_KEY, PROJECTNAME, SCHEDULEDATE, STATUS, CHECK_DATE, MAX_SPEED, UPLOAD_MAX_SPEED, DELIVERED_DATE, RESULT_FILE_DATE, RESULT_SOURCE, BUDGET_YEAR, BUDGET_MONTH, BUDGET_TYPE, PROJECT_TYPE, ENGAGEMENT_TYPE, DELIVERED_YEAR, DELIVERED_MONTH FROM V4_CTELL.PROC.WIP_CURRENT WHERE BUDGET_YEAR >= 2024;

UPDATE SCIERA_SUBSYSTEM.QUICKSIGHT.WIP_RESULTS_DASHBOARD A
SET A.RESULT_SOURCE = B.QUEUED_TO
FROM SCIERA_SUBSYSTEM.SKY_OP.CTELL_PRE_QUEUE_MASTER B
WHERE UPPER(A.MSO) = UPPER(B.MSO) AND UPPER(A.FP_KEY) = UPPER(B.FP_KEY) AND UPPER(A.PROJECTNAME) = UPPER(B.PROJECT_NAME) AND UPPER(A.SERVICENAME) = UPPER(B.SERVICE_PROVIDER) AND A.RESULT_SOURCE IS NULL;
